...
### Если нет наследования типа *

То можно использовать быстрый и новый insert
```c++
vector<vector<int>> lists = {{1,2}, {3,4}, {5,6}};
vector<int> result;

for(const auto& inner_vec : lists) {
    result.insert(result.end(), inner_vec.begin(), inner_vec.end());
}

```

### Если есть наследование (на примере задачи)
